...
OUTPUT_FILE_FORMAT = './example_results_impact__CAIDA{}_sims{}_hijackType{}_rnd_monitors_and_RC_and_RA__Serial_Hijackers.csv'
...
Serial_Hijackers_file = '../data/serial_hijackers/groundtruth_dataset_cropped.csv'
...
list_of_serial_hijackers = []
with open(Serial_Hijackers_file, 'r') as f:
	cr = csv.reader(f,delimiter=',')
	next(cr, None) # skip the first row (i.e., the header)
	for r in cr:
		if int(r[1]) == 1:
			list_of_serial_hijackers.append(int(r[0]))

list_of_serial_hijackers = list(set(list_of_serial_hijackers).intersection(set(list_of_ASNs)))
...
for i in range(nb_of_sims):
	...
	legitimate_AS = random.sample(list_of_ASNs,1)[0]
	hijacker_AS = random.sample([sh for sh in list_of_serial_hijackers if sh !=legitimate_AS],1)[0]
